...
const loadImage = () =>{
    ...
    previewImg.addEventListener("load", () => {
        resetFilterBtn.click(); //clicking the reset btn, so the filter value reset if user select new img
...
rotateOptions.forEach(option => {
    option.addEventListener("click", () => { //adding click event listener to all rotate/flip buttons
        if(option.id === "left"){
            rotate -= 90; //if clicked btn is left rotation, decrement rotate value by -90
        } else if(option.id === "right"){
            rotate += 90; //if clicked btn is right rotation, increment rotate value by +90
        }else if(option.id === "horizontal"){
            //if fliphorizontal value is 1, set this value -1 else set 1 
            flipHorizontal = flipHorizontal === 1 ? -1 : 1;
        }else {
            //if flipVertical value is 1, set this value -1 else set 1 
...
const saveImage = () => {
    ...
    canvas.width = previewImg.naturalWidth; //setting a canvas width to actual image width
    canvas.height = previewImg.naturalHeight; //setting a canvas height to actual image height
